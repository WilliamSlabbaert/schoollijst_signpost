	...
	if ($result->num_rows > 0) {
		while($row = $result->fetch_assoc()) {
			...
			$sql5 = "SELECT *, orders.id as orderid,
				( SELECT GROUP_CONCAT(serialnumber) FROM `decomissioned` WHERE orderid = orders.id AND removed_from_exact is null) as serialnumbers
				FROM orders
			...
			$sql4 = "SELECT *, orders.id as orderid,
				( select count(*) from decomissioned where orderid = orders.id AND removed_from_exact is null ) as amountofdecom,
			...
			$sql3 = "SELECT *, orders.id as orderid,
				( SELECT GROUP_CONCAT(concat(label, ';', serialnumber)) FROM `decomissioned` WHERE orderid = orders.id AND removed_from_exact is null ) as labelnumbers
				FROM orders
				LEFT JOIN schools ON schools.synergyid = orders.synergyid
				LEFT JOIN devices ON devices.spsku = orders.spsku
				WHERE orders.id ='" . $orderid . "' and orders.deleted != 1";
